...
import { useTab4Context } from './Tab4Context';
...
export function Tab4({ sharedData, inputTagsData }: Tab4Props) {
  // Use Tab4 specific context
  const { dataHook, actionsHook } = useTab4Context();
      ...
      {dataHook.noDataFound && (
        <div className="mb-6 p-4 bg-orange-50 dark:bg-orange-900/20 rounded-lg border border-orange-200 dark:border-orange-700/50">
          <p className="text-sm text-orange-800 dark:text-orange-300">
            ⚠️ No input tags found for Tab 4 (m_input = 4). There may be no data available for this configuration.
          </p>
        </div>
      )}
