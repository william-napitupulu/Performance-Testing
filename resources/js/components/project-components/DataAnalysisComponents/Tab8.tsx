...
import { useTab8Context } from './Tab8Context';
...
export function Tab8({ sharedData, inputTagsData }: Tab8Props) {
  // Use Tab8 specific context
  const { dataHook, actionsHook } = useTab8Context();
      ...
      {dataHook.noDataFound && (
        <div className="mb-6 p-4 bg-orange-50 dark:bg-orange-900/20 rounded-lg border border-orange-200 dark:border-orange-700/50">
          <p className="text-sm text-orange-800 dark:text-orange-300">
            ⚠️ No input tags found for Tab 8 (m_input = 8). There may be no data available for this configuration.
          </p>
        </div>
      )}
