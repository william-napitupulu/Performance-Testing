...
import { ArrowLeft, BarChart3, Cog, Database, Play, Plus, Settings } from 'lucide-react';
...
export function DataAnalysisContainer() {
    ...
    const handleDataUpdate = useCallback(
        ...
        [initialDataLoaded, stablePerfId],
    ); // Use stable performance ID
    ...
    const renderTabContent = (tabId: string) => {
        if (tabId === 'new-performance') {
            return <NewPerformanceTestTab onSubmit={handleNewPerformanceSubmit} loading={loading} sharedData={sharedData} />;
        }
        ...
        if (tabId === 'run') {
            return <RunTab sharedData={sharedData} dcsData={data} tabInputData={apiResponse.input_tags} totalRecords={data.length} />;
        }
    ...
    return (
        <div className="bg-background p-6">
            ...
            <div className="tab-content">
                {activeTab && <React.Fragment key={`${activeTab}-${sharedData.perfId || 'no-perf'}`}>{renderTabContent(activeTab)}</React.Fragment>}
            </div>
        </div>
    );
}
