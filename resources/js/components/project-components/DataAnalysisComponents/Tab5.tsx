...
import { useTab5Context } from './Tab5Context';
...
export function Tab5({ sharedData, inputTagsData }: Tab5Props) {
  // Use Tab5 specific context
  const { dataHook, actionsHook } = useTab5Context();

  return (
    <div className="p-6 bg-background rounded-b-lg border border-border dark:border-border/50">
      {/* Header Section */}
      <div className="bg-indigo-50/70 dark:bg-indigo-900/10 rounded-lg p-6 border border-indigo-100 dark:border-indigo-800/50 mb-6">
        <h2 className="text-xl font-semibold text-indigo-700 dark:text-indigo-300 mb-2">
          Tab 5 - Manual Input (m_input = 5)
        </h2>
      </div>
      ...
      {dataHook.noDataFound && (
        <div className="mb-6 p-4 bg-orange-50 dark:bg-orange-900/20 rounded-lg border border-orange-200 dark:border-orange-700/50">
          <p className="text-sm text-orange-800 dark:text-orange-300">
            ⚠️ No input tags found for Tab 5 (m_input = 5). There may be no data available for this configuration.
          </p>
        </div>
      )}
