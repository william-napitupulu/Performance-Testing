...
export function DataAnalysisTableHeader({ sortField, sortDirection, onSort }: DataAnalysisTableHeaderProps) {
    ...
    return (
        <thead className="border-y border-blue-100 bg-blue-50/70 dark:border-gray-700 dark:bg-gray-800/70">
            <tr>
                <th className={`${headerBaseClasses} w-30 px-4 ${getHeaderColor('no')}`} onClick={() => onSort('no')}>
                    <div className="flex justify-center">No {renderSortIcon('no')}</div>
                </th>
                ...
                <th className={`${headerBaseClasses} px-6 ${getHeaderColor('value')}`} onClick={() => onSort('value')}>
                    <div className="flex justify-center">Value {renderSortIcon('value')}</div>
                </th>
                <th className={`${headerBaseClasses} px-6 ${getHeaderColor('date_rec')}`} onClick={() => onSort('date_rec')}>
                    <div className="flex items-center">Date {renderSortIcon('date_rec')}</div>
                </th>
            </tr>
        </thead>
    );
