...
export interface Tab2ContextType {
  dataHook: ReturnType<typeof useTab1Data>;
  actionsHook: ReturnType<typeof useTab1Actions>;
  mInput: number;
}

const Tab2Context = createContext<Tab2ContextType | undefined>(undefined);
...
export const Tab2Provider: React.FC<Tab2ProviderProps> = ({ sharedData, children, inputTagsData, onDataSaved }) => {
  ...
  return (
    <Tab2Context.Provider value={{ dataHook, actionsHook, mInput }}>
      {children}
    </Tab2Context.Provider>
  );
};

export const useTab2Context = (): Tab2ContextType => {
  const ctx = useContext(Tab2Context);
  if (!ctx) {
    throw new Error('useTab2Context must be used within a Tab2Provider');
  }
  return ctx;
}; 