...
import { useTab2Context } from './Tab2Context';
...
export function Tab2({ sharedData, inputTagsData }: Tab2Props) {
  // Use Tab2 specific context
  const { dataHook, actionsHook } = useTab2Context();

  return (
    <div className="p-6 bg-background rounded-b-lg border border-border dark:border-border/50">
      {/* Header Section */}
      <div className="bg-orange-50/70 dark:bg-orange-900/10 rounded-lg p-6 border border-orange-100 dark:border-orange-800/50 mb-6">
        <h2 className="text-xl font-semibold text-orange-700 dark:text-orange-300 mb-2">
          Tab 2 - Manual Input (m_input = 2)
        </h2>
      ...
      {/**
       * PerformanceSelector (dropdown) is no longer needed because we arrive at Tab2
        ...
        <div className="mb-6 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
          <div className="flex items-center justify-center">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Loading input tags for Tab 2 (m_input = 2)...
            </p>
          </div>
        </div>
      )}
        ...
        <div className="mb-6 p-4 bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-700/50">
          <div className="flex items-center justify-between">
            <p className="text-sm text-red-800 dark:text-red-300">
              ⏰ Request timed out after 10 seconds. Unable to load input tags for Tab 2 (m_input = 2).
            </p>
            <button
        ...
        <div className="mb-6 p-4 bg-orange-50 dark:bg-orange-900/20 rounded-lg border border-orange-200 dark:border-orange-700/50">
          <div className="flex items-center justify-between">
            <p className="text-sm text-orange-800 dark:text-orange-300">
              ⚠️ No input tags found for Tab 2 (m_input = 2). There may be no data available for this configuration.
            </p>
            <button
              onClick={() => dataHook.fetchInputTags(sharedData.dateTime, sharedData.perfId)}
