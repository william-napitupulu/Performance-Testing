-- MariaDB compatible queries for tb_performance table
-- MariaDB is MySQL-compatible, so most syntax remains the same

-- 1. SELECT query (fixed column name typo: date_perfomance -> date_performance)
SELECT * FROM pt.tb_performance 
WHERE unit_id = 1 
ORDER BY date_performance DESC, perf_id DESC;

-- 2. DROP TABLE (same syntax in MariaDB)
DROP TABLE tb_performance;

-- 3. SELECT ALL (same syntax in MariaDB)
SELECT * FROM tb_performance;

-- 4. DELETE ALL records (corrected syntax - removed asterisk)
DELETE FROM tb_performance;

-- Alternative: TRUNCATE is faster for clearing all data
TRUNCATE TABLE tb_performance;

-- ========================================================
-- Mock data for pt.tb_performance table (MariaDB compatible)
-- Insert sample performance testing records
-- Status: 1 = Editable, 0 = Locked
-- ========================================================

INSERT INTO pt.tb_performance (perf_id, description, date_performance, date_created, status, unit_id) VALUES
...
-- ========================================================
-- MariaDB-specific optimizations (optional)
-- ========================================================

-- Create index for better performance (MariaDB syntax)
CREATE INDEX idx_unit_date ON tb_performance (unit_id, date_performance DESC);
CREATE INDEX idx_status ON tb_performance (status);

-- Check MariaDB version compatibility
SELECT VERSION() as mariadb_version;

-- Show table structure (MariaDB compatible)
DESCRIBE tb_performance;
SHOW CREATE TABLE tb_performance; 